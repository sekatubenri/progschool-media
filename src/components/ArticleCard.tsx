...
const CATEGORY_STYLES: Record<string, { bg: string; icon: string; iconBg: string }> = {
  school: { bg: 'from-violet-600 to-purple-700', icon: '🏫', iconBg: 'bg-white/20' },
  language: { bg: 'from-cyan-600 to-blue-700', icon: '💻', iconBg: 'bg-white/20' },
  career: { bg: 'from-orange-500 to-red-600', icon: '🚀', iconBg: 'bg-white/20' },
  beginner: { bg: 'from-green-500 to-emerald-600', icon: '🌱', iconBg: 'bg-white/20' },
}

export default function ArticleCard({ article }: { article: Article }) {
  const style = CATEGORY_STYLES[article.category] || CATEGORY_STYLES.school
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        <h3 className="font-bold text-gray-900 text-sm leading-relaxed mb-3 line-clamp-3 group-hover:text-violet-700 transition-colors">
          {article.title}
        </h3>
